...
def animate_deformation(points: np.ndarray, connectivity: np.ndarray, deformations: np.ndarray, output_file: str):
    """
...
XDMF_FILE = "./deformed_bunny_files_tunned/displacement_1.xdmf"
...
print(f"\ntype(time_steps) = {type(time_steps)}")
print(f"np.shape(time_steps) = {np.shape(time_steps)}")
print(f"time_steps = \n{time_steps}\n")

print(f"\ntype(deformations) = {type(deformations)}")
print(f"np.shape(deformations) = {np.shape(deformations)}")
print(f"deformations = \n{deformations}\n")


# Animate deformation
animate_deformation(points, connectivity, deformations, OUTPUT_FILE)
