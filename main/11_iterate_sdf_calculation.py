...
# Updated validate_indices function
...
def main():
    ...
    # Load the filtered points, These are the points of fingers. The filtering is for the location of the finger pressure is applied
    filtered_points = np.loadtxt(FILTERED_POINTS_FILE, skiprows=1)
    print(f"Loaded {len(filtered_points)} filtered points.")

    if args.doall:
        indices_to_traverse = np.arange(len(filtered_points))  # Generate indices for all filtered points
    else:
        ...
        valid_indices = np.loadtxt(VALID_INDICES_FILE, skiprows=1, dtype=int)
            # Determine indices to traverse based on the filter

        if args.filter == "all":
            indices_to_traverse = np.sort(np.unique(np.concatenate((buggy_indices, valid_indices))))
        elif args.filter == "buggy":
            indices_to_traverse = np.sort(buggy_indices)
        elif args.filter == "valid":
            indices_to_traverse = np.sort(valid_indices)

      

    print(f"We'll be traversing the indices {indices_to_traverse}")
    if len(indices_to_traverse) == 0:
        print("len(indices_to_traverse) == 0, so there's no file to go through") 
        return 3
    ...
    # Validate indices
    validate_indices(starting_index, stopping_index, indices_to_traverse)

    if stopping_index == -1:
        stopping_index = len(indices_to_traverse) -1
