...
if STOPPING_INDEX < STARTING_INDEX:
    raise ValueError(
        f"STOPPING_INDEX ({STOPPING_INDEX}) cannot be less than STARTING_INDEX ({STARTING_INDEX})."
    )

print(
    f"Processing range: STARTING_INDEX={STARTING_INDEX}, STOPPING_INDEX={STOPPING_INDEX}"
)

# Iterate over each row and run the simulation
for index, finger_position in enumerate(
    filtered_points[STARTING_INDEX : STOPPING_INDEX + 1], start=STARTING_INDEX
):
    # Convert finger position to string for passing as arguments
    finger_position_str = " ".join(map(str, finger_position))

    print(
        f"Running simulation for index: {index}, finger_position: {finger_position_str}"
    )
